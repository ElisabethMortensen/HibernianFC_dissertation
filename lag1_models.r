...
lmer_step_1 <- step(lmer_full, reduce.random = FALSE)
lmer_best_1 <- get_model(lmer_step_1)
print(lmer_step_1)

lmer_train_1 <- train_metrics_lmer(lmer_best_1)
lmer_test_1 <- test_metrics_lmer(lmer_best_1, test_h2)

cat(sprintf("AIC: %.2f",  lmer_train_1$AIC))
cat(sprintf("Train RMSE:%.4f", lmer_train_1$Train_RMSE))
cat(sprintf("Train MAE:%.4f", lmer_train_1$Train_MAE))
cat(sprintf("Test RMSE:%.4f", lmer_test_1$Test_RMSE))
cat(sprintf("Test MAE:%.4f", lmer_test_1$Test_MAE))
cat(sprintf("Test R²:%.4f", lmer_test_1$Test_R2))
...
lmer_quad_step_1 <- step(lmer_quad_full, reduce.random = FALSE)
lmer_quad_best_1 <- get_model(lmer_quad_step_1)
print(lmer_quad_step_1)

lmer_quad_train_1 <- train_metrics_lmer(lmer_quad_best_1)
lmer_quad_test_1 <- test_metrics_lmer(lmer_quad_best_1, test_h2)

cat(sprintf("AIC:%.2f",  lmer_quad_train_1$AIC))
cat(sprintf("Train RMSE:%.4f", lmer_quad_train_1$Train_RMSE))
cat(sprintf("Train MAE:%.4f", lmer_quad_train_1$Train_MAE))
cat(sprintf("Test RMSE:%.4f", lmer_quad_test_1$Test_RMSE))
cat(sprintf("Test MAE:%.4f", lmer_quad_test_1$Test_MAE))
cat(sprintf("Test R²:%.4f", lmer_quad_test_1$Test_R2))
...
lmer_cub_step_1 <- step(lmer_cub_full, reduce.random = FALSE)
lmer_cub_best_1 <- get_model(lmer_cub_step_1)
print(lmer_cub_step_1)

lmer_cub_train_1 <- train_metrics_lmer(lmer_cub_best_1)
lmer_cub_test_1 <- test_metrics_lmer(lmer_cub_best_1, test_h2) 

cat(sprintf("AIC:%.2f", lmer_cub_train_1$AIC))
cat(sprintf("Train RMSE:%.4f", lmer_cub_train_1$Train_RMSE))
cat(sprintf("Train MAE:%.4f", lmer_cub_train_1$Train_MAE))
cat(sprintf("Test RMSE:%.4f", lmer_cub_test_1$Test_RMSE))
cat(sprintf("Test MAE:%.4f", lmer_cub_test_1$Test_MAE))
cat(sprintf("Test R²: %.4f", lmer_cub_test_1$Test_R2))
...
sm_gauss <- summary(gam_full_gauss)
gauss_train_1 <- train_metrics_gam(gam_full_gauss, sm_gauss)
gauss_test_1 <- test_metrics_gam(gam_full_gauss, test_h2)

print(sm_gauss)
cat(sprintf("AIC: %.2f", gauss_train_1$AIC))
cat(sprintf("Deviance expl: %.1f%%", gauss_train_1$Dev_Expl))
cat(sprintf("Train RMSE: %.4f", gauss_train_1$Train_RMSE))
cat(sprintf("Train MAE: %.4f", gauss_train_1$Train_MAE))
cat(sprintf("Test RMSE: %.4f", gauss_test_1$Test_RMSE))
cat(sprintf("Test MAE: %.4f", gauss_test_1$Test_MAE))
cat(sprintf("Test R²: %.4f", gauss_test_1$Test_R2))

gam.check(gam_full_gauss)
plot(gam_full_gauss, pages=1, shade=TRUE, shade.col="#1565C020", seWithMean=TRUE, scale=0,rug=TRUE)
...
sm_scat <- summary(gam_full_scat)
scat_train_1 <- train_metrics_gam(gam_full_scat, sm_scat)
scat_test_1 <- test_metrics_gam(gam_full_scat, test_h2)
print(sm_scat)
cat(sprintf("AIC: %.2f", scat_train_1$AIC))
cat(sprintf("Deviance expl: %.1f%%", scat_train_1$Dev_Expl))
cat(sprintf("Train RMSE: %.4f", scat_train_1$Train_RMSE))
cat(sprintf("Train MAE: %.4f", scat_train_1$Train_MAE))
cat(sprintf("Test RMSE: %.4f", scat_test_1$Test_RMSE))
cat(sprintf("Test MAE: %.4f", scat_test_1$Test_MAE))
cat(sprintf("Test R²: %.4f", scat_test_1$Test_R2))


gam.check(gam_full_scat)
plot(gam_full_scat, pages=1, shade=TRUE, shade.col="#1565C020", seWithMean=TRUE, rug=TRUE, scale=0)
...
comparison <- data.frame(
  ...
  AIC = c(lmer_train_1$AIC, lmer_quad_train_1$AIC, lmer_cub_train_1$AIC, gauss_train_1$AIC, scat_train_1$AIC),
  Train_RMSE = c(lmer_train_1$Train_RMSE, lmer_quad_train_1$Train_RMSE, lmer_cub_train_1$Train_RMSE, gauss_train_1$Train_RMSE, scat_train_1$Train_RMSE),
  Train_MAE = c(lmer_train_1$Train_MAE, lmer_quad_train_1$Train_MAE, lmer_cub_train_1$Train_MAE, gauss_train_1$Train_MAE, scat_train_1$Train_MAE),
  Dev_Expl = c(NA, NA, NA, gauss_train_1$Dev_Expl, scat_train_1$Dev_Expl),
  Test_RMSE = c(lmer_test_1$Test_RMSE, lmer_quad_test_1$Test_RMSE, lmer_cub_test_1$Test_RMSE, gauss_test_1$Test_RMSE, scat_test_1$Test_RMSE),
  Test_MAE = c(lmer_test_1$Test_MAE, lmer_quad_test_1$Test_MAE, lmer_cub_test_1$Test_MAE, gauss_test_1$Test_MAE, scat_test_1$Test_MAE)
)
...
#run diagnostic plots for all models

# Linear mixed effect
preds_linear <- get_preds_lmer(lmer_best_1, test_h2)
plot_diagnostics(preds_linear, "Linear Mixed Effects", lag_minutes)

# Quadratic mixed effect
preds_quad <- get_preds_lmer(lmer_quad_best_1, test_h2)
plot_diagnostics(preds_quad, "Quadratic Mixed Effects", lag_minutes)

# Cubic mixed effect
preds_cub <- get_preds_lmer(lmer_cub_best_1, test_h2)
plot_diagnostics(preds_cub, "Cubic Mixed Effects", lag_minutes)

# GAM Gaussian
preds_gauss <- get_preds_gam(gam_full_gauss, test_h2)
plot_diagnostics(preds_gauss, "GAM Gaussian", lag_minutes)

# GAM Scaled T
preds_scat <- get_preds_gam(gam_full_scat, test_h2)
plot_diagnostics(preds_scat, "GAM Scaled T", lag_minutes)


# save models
saveRDS(lmer_best_1, "lmer_lin_lag1.rds")
saveRDS(lmer_quad_best_1, "lmer_quad_lag1.rds")
saveRDS(lmer_cub_best_1, "lmer_cub_lag1.rds")
saveRDS(gam_full_gauss, "gam_gauss_lag1.rds")
saveRDS(gam_full_scat, "gam_scat_lag1.rds")
