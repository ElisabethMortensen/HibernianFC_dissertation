...
saveRDS(test_h2, "test_h2_lag5.rds")


# train metrics for lmer
...
lmer_step_5 <- step(lmer_full_5, reduce.random = FALSE)
lmer_best_5 <- get_model(lmer_step_5)
print(lmer_step_5)

lmer_train_5 <- train_metrics_lmer(lmer_best_5)
lmer_test_5 <- test_metrics_lmer(lmer_best_5, test_h2)

cat(sprintf("AIC: %.2f",  lmer_train_5$AIC))
cat(sprintf("Train RMSE:%.4f", lmer_train_5$Train_RMSE))
cat(sprintf("Train MAE:%.4f", lmer_train_5$Train_MAE))
cat(sprintf("Test RMSE:%.4f", lmer_test_5$Test_RMSE))
cat(sprintf("Test MAE:%.4f", lmer_test_5$Test_MAE))
cat(sprintf("Test R²:%.4f", lmer_test_5$Test_R2))




#Quadratic mixed effects
lmer_quad_full_5 <- lmer(
  deterioration_next ~
    deterioration_lag +
    match_minute +
    match_minute2 +
...
lmer_quad_step_5 <- step(lmer_quad_full_5, reduce.random = FALSE)
lmer_quad_best_5 <- get_model(lmer_quad_step_5)
print(lmer_quad_step_5)

lmer_quad_train_5 <- train_metrics_lmer(lmer_quad_best_5)
lmer_quad_test_5 <- test_metrics_lmer(lmer_quad_best_5, test_h2)

cat(sprintf("AIC:%.2f", lmer_quad_train_5$AIC))
cat(sprintf("Train RMSE:%.4f", lmer_quad_train_5$Train_RMSE))
cat(sprintf("Train MAE:%.4f", lmer_quad_train_5$Train_MAE))
cat(sprintf("Test RMSE:%.4f", lmer_quad_test_5$Test_RMSE))
cat(sprintf("Test MAE:%.4f", lmer_quad_test_5$Test_MAE))
cat(sprintf("Test R²:%.4f", lmer_quad_test_5$Test_R2))
...
lmer_cub_step_5 <- step(lmer_cub_full_5, reduce.random = FALSE)
lmer_cub_best_5 <- get_model(lmer_cub_step_5)
print(lmer_cub_step_5)

lmer_cub_train_5 <- train_metrics_lmer(lmer_cub_best_5)
lmer_cub_test_5 <- test_metrics_lmer(lmer_cub_best_5, test_h2) 

cat(sprintf("AIC:%.2f", lmer_cub_train_5$AIC))
cat(sprintf("Train RMSE:%.4f", lmer_cub_train_5$Train_RMSE))
cat(sprintf("Train MAE:%.4f", lmer_cub_train_5$Train_MAE))
cat(sprintf("Test RMSE:%.4f", lmer_cub_test_5$Test_RMSE))
cat(sprintf("Test MAE:%.4f", lmer_cub_test_5$Test_MAE))
cat(sprintf("Test R²: %.4f", lmer_cub_test_5$Test_R2))
...
sm_gauss <- summary(gam_full_gauss_5)
gauss_train_5 <- train_metrics_gam(gam_full_gauss_5, sm_gauss)
gauss_test_5 <- test_metrics_gam(gam_full_gauss_5, test_h2)

print(sm_gauss)
cat(sprintf("AIC: %.2f", gauss_train_5$AIC))
cat(sprintf("Deviance expl: %.1f%%", gauss_train_5$Dev_Expl))
cat(sprintf("Train RMSE: %.4f", gauss_train_5$Train_RMSE))
cat(sprintf("Train MAE: %.4f", gauss_train_5$Train_MAE))
cat(sprintf("Test RMSE: %.4f", gauss_test_5$Test_RMSE))
cat(sprintf("Test MAE: %.4f", gauss_test_5$Test_MAE))
cat(sprintf("Test R²: %.4f", gauss_test_5$Test_R2))

gam.check(gam_full_gauss_5)
plot(gam_full_gauss_5, pages=1, shade=TRUE, shade.col="#1565C020", seWithMean=TRUE, scale=0,rug=TRUE)
...
sm_scat <- summary(gam_full_scat_5)
scat_train_5 <- train_metrics_gam(gam_full_scat_5, sm_scat)
scat_test_5 <- test_metrics_gam(gam_full_scat_5, test_h2)

print(sm_scat)
cat(sprintf("AIC: %.2f", scat_train_5$AIC))
cat(sprintf("Deviance expl: %.1f%%", scat_train_5$Dev_Expl))
cat(sprintf("Train RMSE: %.4f", scat_train_5$Train_RMSE))
cat(sprintf("Train MAE: %.4f", scat_train_5$Train_MAE))
cat(sprintf("Test RMSE: %.4f", scat_test_5$Test_RMSE))
cat(sprintf("Test MAE: %.4f", scat_test_5$Test_MAE))
cat(sprintf("Test R²: %.4f", scat_test_5$Test_R2))


plot(gam_full_scat_5, pages=1, shade=TRUE, shade.col="#1565C020", seWithMean=TRUE, rug=TRUE, scale=0)
...
comparison <- data.frame(
  ...
  AIC = c(lmer_train_5$AIC, lmer_quad_train_5$AIC, lmer_cub_train_5$AIC, gauss_train_5$AIC, scat_train_5$AIC),
  Train_RMSE = c(lmer_train_5$Train_RMSE, lmer_quad_train_5$Train_RMSE, lmer_cub_train_5$Train_RMSE, 
                 gauss_train_5$Train_RMSE, scat_train_5$Train_RMSE),
  Train_MAE = c(lmer_train_5$Train_MAE, lmer_quad_train_5$Train_MAE, lmer_cub_train_5$Train_MAE, 
                gauss_train_5$Train_MAE, scat_train_5$Train_MAE),
  Dev_Expl = c(NA, NA, NA, gauss_train_5$Dev_Expl, scat_train_5$Dev_Expl),
  Test_RMSE = c(lmer_test_5$Test_RMSE, lmer_quad_test_5$Test_RMSE, lmer_cub_test_5$Test_RMSE, 
                 gauss_test_5$Test_RMSE, scat_test_5$Test_RMSE),
  Test_MAE = c(lmer_test_5$Test_MAE, lmer_quad_test_5$Test_MAE, lmer_cub_test_5$Test_MAE, 
                 gauss_test_5$Test_MAE, scat_test_5$Test_MAE)
)
...
get_preds_lmer <- function(model, test_data) {
  preds <- tryCatch(
    predict(model, newdata = test_data, re.form = ~(1|athlete_id), allow.new.levels = TRUE),
  ...
  preds <- as.numeric(preds)
  ok <- !is.na(actual) & !is.na(preds) & is.finite(preds)
  list( actual = actual[ok], preds = preds[ok], resids = actual[ok] - preds[ok], minutes = test_data$match_minute[ok]
  )
}

get_preds_gam <- function(model, test_data) {
  preds <- tryCatch(
    predict(model, newdata = test_data, type = "response", exclude = "s(match_id)", newdata.guaranteed = TRUE),
    error = function(e) NULL
  )
  if (is.null(preds)) return(NULL)
  actual <- as.numeric(test_data$deterioration_next)
...
#run disagnostics for all models
# Linear
preds_linear <- get_preds_lmer(lmer_best_5, test_h2)
plot_diagnostics(preds_linear, "Linear Mixed Effects", lag_minutes)

# Quadratic
preds_quad <- get_preds_lmer(lmer_quad_best_5, test_h2)
plot_diagnostics(preds_quad, "Quadratic Mixed Effects", lag_minutes)

# Cubic
preds_cub <- get_preds_lmer(lmer_cub_best_5, test_h2)
plot_diagnostics(preds_cub, "Cubic Mixed Effects", lag_minutes)

# GAM Gaussian
preds_gauss <- get_preds_gam(gam_full_gauss_5, test_h2)
plot_diagnostics(preds_gauss, "GAM Gaussian", lag_minutes)

# GAM Scaled T
preds_scat <- get_preds_gam(gam_full_scat_5, test_h2)
plot_diagnostics(preds_scat, "GAM Scaled T", lag_minutes)


#save models
saveRDS(lmer_best_5, "lmer_lin_lag5.rds")
saveRDS(lmer_quad_best_5, "lmer_quad_lag5.rds")
saveRDS(lmer_cub_best_5, "lmer_cub_lag5.rds")
saveRDS(gam_full_gauss_5, "gam_gauss_lag5.rds")
saveRDS(gam_full_scat_5, "gam_scat_lag5.rds")